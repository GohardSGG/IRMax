use std::sync::Weak;
use std::{cell::RefCell, mem::size_of, num::NonZeroIsize, ptr::{null, null_mut}, sync::{atomic::{AtomicBool, AtomicUsize, Ordering}, Arc}, time::Duration};
...
use windows::Win32::UI::{Controls::WM_MOUSELEAVE, Input::{Ime::{GCS_RESULTSTR, ImmGetCompositionStringW, ImmGetContext, ImmReleaseContext}, KeyboardAndMouse::{GetAsyncKeyState, SetCapture, SetFocus, TrackMouseEvent, TME_LEAVE, TRACKMOUSEEVENT, VIRTUAL_KEY, VK_CONTROL, VK_MENU, VK_SHIFT}}, WindowsAndMessaging::{CallNextHookEx, CreateWindowExW, DefWindowProcW, DestroyWindow, GetWindowLongPtrW, LoadCursorW, MoveWindow, PostMessageW, RegisterClassW, SendMessageW, SetCursor, SetCursorPos, SetWindowLongPtrW, SetWindowsHookExW, ShowCursor, UnhookWindowsHookEx, UnregisterClassW, CS_OWNDC, DLGC_WANTALLKEYS, DLGC_WANTCHARS, GWLP_USERDATA, HHOOK, HICON, IDC_ARROW, MOUSEHOOKSTRUCTEX, WH_MOUSE, WINDOW_EX_STYLE, WM_CHAR, WM_GETDLGCODE, WM_IME_CHAR, WM_IME_COMPOSITION, WM_KEYDOWN, WM_KEYUP, WM_LBUTTONDOWN, WM_LBUTTONUP, WM_MBUTTONDOWN, WM_MBUTTONUP, WM_MOUSEMOVE, WM_MOUSEWHEEL, WM_MOVE, WM_RBUTTONDOWN, WM_RBUTTONUP, WNDCLASSW, WS_CHILD, WS_VISIBLE}};
...
use super::{cursors::Cursors, drop_target::DropTarget, keyboard::virtual_key_to_keycode, message_window::MessageWindow, to_wstr, version::is_windows10_or_greater, PLUGIN_HINSTANCE, WM_USER_CHAR, WM_USER_FRAME_TIMER, WM_USER_KEY_DOWN, WM_USER_KEY_UP};
...
impl OsWindowInterface for OsWindow {
    ...
    fn set_input_focus(&self, focus: bool) {
        if focus {
            ...
            self.message_window.set_focus(false);
        }
    }
...
fn dispatch_utf16_text(window: &OsWindow, utf16: &[u16]) {
    if utf16.is_empty() {
        return;
    }

    fn dispatch_control_char(window: &OsWindow, character: char) -> bool {
        let key_code = match character {
            '\u{0008}' => Code::Backspace,
            '\u{0009}' => Code::Tab,
            '\u{000A}' | '\u{000D}' => Code::Enter,
            '\u{001B}' => Code::Escape,
            '\u{007F}' => Code::Delete,
            _ => return false,
        };
    ...
    for character in text.chars() {
        ...
        // Windows may deliver control characters in WM_CHAR (for example,
        // VK_SHIFT=0x10, VK_BACK=0x08 via translated messages).
        // We handle functional keys through WM_KEYDOWN/WM_KEYUP and should not
        // inject these controls as editable text.
        if character.is_control() {
            let _ = dispatch_control_char(window, character);
            continue;
        }
...
fn dispatch_ime_result(window: &OsWindow, hwnd: HWND) -> bool {
    ...
    if bytes_required > 0 {
        ...
        if unit_count > 0 {
            ...
            if copied > 0 {
                ...
                dispatch_utf16_text(window, &utf16[..copied_units]);
                dispatched = true;
            }
        }
    }
    ...
    let result = if let Some(window) = window_weak.upgrade() {
        match msg {
            ...
            WM_CHAR | WM_IME_CHAR => {
                ...
                dispatch_utf16_text(window.as_ref(), &[utf16_unit]);
                LRESULT(0)
            },
            ...
            WM_IME_COMPOSITION => {
                ...
                if composition_flags & GCS_RESULTSTR.0 != 0 {
                    if dispatch_ime_result(window.as_ref(), hwnd) {
                        return LRESULT(0);
                    }
                }
            ...
            WM_KEYDOWN => {
                window.send_event(Event::KeyDown {
                    key_code: virtual_key_to_keycode(VIRTUAL_KEY(wparam.0 as _)),
                    text: None,
                });
            ...
            WM_USER_KEY_DOWN => {
                window.send_event(Event::KeyDown {
                    key_code: virtual_key_to_keycode(VIRTUAL_KEY(wparam.0 as u16)),
                    text: None,
                });
